...
import classeshort
from  neatdrive.ReturnUltimoCheckPoint  import *
...
def eval_genome(genome, config, ListaClosePrice):
    ...
    if contShort > len(ultimoindicelist) and fitness>9:
        print(f"ContiLong:{contLong} Valor tem quer se 0!")
...
def run(config_file, CarregarWin=False, CarregarCheckPoint=True):
    ...
    if CarregarCheckPoint:
        p = neat.Checkpointer.restore_checkpoint(
            returnultimoCheckPoint('./neatdrive/CheckPoints')
        )
    else:
...
if __name__ == "__main__":
    ...
    config_path = os.path.join(local_dir, "neatdrive/configteste.txt")
    run(config_path)
