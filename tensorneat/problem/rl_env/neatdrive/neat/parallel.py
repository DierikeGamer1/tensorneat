...
class ParallelEvaluator(object):
    ...
    def evaluate(self, genomes, config):
        ...
        if "ContinuarTreinandoTemporario.py" in nome_arquivo or "ContinuarShort.py" in nome_arquivo or "testejax.py" in nome_arquivo:
            ...
            for numero, nomearquivo in enumerate(lista):
                if "adausdt" in nomearquivo:
                    if "adausdt2024-02-20-00-21-41-425423" in nomearquivo:
                        with open(f"neatdrive/DadosTreino/{nomearquivo}", "r") as nomearquivo:
                            Listadados = []
                            Listadados.append(
                                [
                                    json.loads(
                            ...
                            for i in range(1, int(div)):
                                ListaClosePrice.append(
                                    ListaClosePrice[cgh][t_DeInterval * i + 1 - t_DeInterval : t_DeInterval * i]
                                )
